...
> **Schema URL compatibility:** For OCX CLI-supported manifests, `$schema` is required and must be `https://ocx.kdco.dev/schemas/v2/registry.json`. Registries may still serve `https://ocx.kdco.dev/schemas/registry.json` as an alias/latest pointer for compatibility, but OCX CLI treats registry manifests with a missing `$schema` or the unversioned URL as legacy v1 and rejects them.
