...
- other non-zero codes — runtime/tool failures (for example missing files, I/O, or internal errors)
...
Suppresses all output (including errors). Check the exit code to determine outcome (`0` for valid, `78` for configuration/validation failures, other non-zero for runtime/tool failures).
